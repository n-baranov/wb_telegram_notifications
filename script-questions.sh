...
curl -s --location --request GET 'https://feedbacks-api.wb.ru/api/v1/questions?isAnswered=false&take=5000&skip=0' --header 'Authorization: '$WB_TOKEN --header 'Content-Type: application/json' | jq -r '.data.questions' > $CURRENT_FILEPATH/$CURRENT_FILENAME
    ...
    if [ "$NOT_ANSWERED_JSON" != "[]" ] && [ "$NOT_ANSWERED_JSON" != null ] && [ "$NOT_ANSWERED_JSON" != "" ]; then
	    ...
	    CHANGES_COUNT=$(jq length $CURRENT_FILEPATH/$CURRENT_FILENAME)
            ...
            do
                  ...
                  JSON_DATE=$(jq -r '.['$j'].createdDate' $CURRENT_FILEPATH/$CURRENT_FILENAME)
                  ...
                  for k in `seq 1 $CHAT_NUM`; do
	                	...
	                	curl -s -X POST 'https://api.telegram.org/bot'$TG_BOT_TOKEN'/sendMessage' -d chat_id=${!current_chat_id} \
             	                        -d text="Новый вопрос! %0A$LK_NAME%0AТекст: $JSON_TEXT" &
                                echo "--------------------"
                        fi
                  done
            done
